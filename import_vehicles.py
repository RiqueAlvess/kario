    ...
    with open(csv_file_path, 'r', encoding='utf-8') as csvfile:
        ...
        for row_num, row in enumerate(reader, start=3):
            try:
                ...
                # Verificar se VIN já existe (é unique no banco)
                # Se já existir, deixar vazio para evitar erro
                if vin and Vehicle.objects.filter(vin=vin).exists():
                    print(f"  ⚠ VIN {vin} duplicado, será deixado vazio")
                    vin = None
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print("Uso: python import_vehicles.py <arquivo.csv>")
        print("\nExemplo:")
        print("  python import_vehicles.py vehicles.csv")
        sys.exit(1)
